...
# This code is meant to be run in chunks from header to header
# after the initialization process. I ran every chunk seperately from
# the others, so filtering in one section may effect other sections.
# Everything starts uncommented for visual purposes,
# but only one section should be run at a time to avoid filtering issues.

# Main data has been redacted per competition rules,
# so none of this code will actually run.
# but the defensive and offensive passing and rushing data are included
# from https://www.nfl.com/stats/team-stats/
...
df = pd.read_csv(__REDACTED__, low_memory=False)

dfDPassing = pd.read_csv("defensivePassingData.csv")
dfDRushing = pd.read_csv("defensiveRushingData.csv")
dfOPassing = pd.read_csv("offensivePassingData.csv")
dfORushing = pd.read_csv("offensiveRushingData.csv")
